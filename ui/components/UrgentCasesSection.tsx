...
const UrgentCasesSection: React.FC<UrgentCasesSectionProps> = ({
  ...
  return <section className="py-12 bg-red-50">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {urgentCases.map(person => <div key={person.id} className="bg-white rounded-xl shadow-md border-2 border-red-200 overflow-hidden hover:shadow-lg transition-shadow">
              <div className="relative">
                <img src={person.photo_url} alt={person.full_name} className="w-full h-56 object-cover" />
                <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-black/50 to-transparent p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-3 py-1 bg-white/90 text-gray-900 text-sm font-medium rounded-full" data-mixed-content="true">
              ...
              <div className="p-5">
                ...
                <p className="text-gray-500 mb-4">{person.age} ans • {person.gender}</p>
                
                <div className="space-y-2 mb-4">
                  <div className="flex items-center text-gray-600 text-sm">
                    <MapPinIcon size={16} className="mr-2 text-red-500 flex-shrink-0" />
                    <span data-mixed-content="true">{person.last_seen_location}, {person.region}</span>
                  </div>
                  <div className="flex items-center text-gray-600 text-sm">
                    <CalendarIcon size={16} className="mr-2 text-red-500 flex-shrink-0" />
                    <span data-mixed-content="true">Disparu(e) le {formatDate(person.last_seen_date)}</span>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => onViewDetails(person)} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors text-sm font-medium">
                    Voir détails
                    <ArrowRightIcon size={16} />
                  </button>
                  <button onClick={() => onContact(person.contact_phone)} className="p-2.5 bg-red-500 hover:bg-red-600 text-white rounded-lg transition-colors" title="Appeler">
                    <PhoneIcon size={18} />
                  </button>
                </div>
              </div>
